...
def test_main(num_tokens: int,
    ...
    try:
        buffer.low_latency_dispatch(
            x_list[0], topk_idx, num_tokens, num_experts,
            use_fp8=True, round_scale=True, use_ue8m0=False, use_mxfp8=True)
    except AssertionError:
        pass
    else:
        raise AssertionError('MXFP8 dispatch should require `use_ue8m0=True`')

    for current_x in x_list:
        for return_recv_hook in (False, True):
            for dispatch_use_fp8, round_scale, use_ue8m0, use_mxfp8 in dispatch_cases:
                ...
                if dispatch_use_fp8:
                    group_size = 32 if use_mxfp8 else 128
                    scales = packed_recv_x[1][:, :packed_recv_x[0].size(1)] if use_mxfp8 else packed_recv_x[1]
                    simulated_gemm_x = per_token_cast_back(
                        packed_recv_x[0].view(-1, hidden),
                        scales.contiguous().view(-1, hidden // group_size),
                        group_size=group_size).view(packed_recv_x[0].shape)
                else:
                    simulated_gemm_x = packed_recv_x.clone()
                for i in range(num_local_experts if do_check else 0):
                    expert_id = rank * num_local_experts + i
                    recv_x = per_token_cast_back(
                        packed_recv_x[0][i],
                        packed_recv_x[1][i],
                        group_size=32 if use_mxfp8 else 128) if dispatch_use_fp8 else packed_recv_x[i]
